...
/*PPMImage *filter(PPMImage *im, int hw)
{
	...
	// arrays storing the rgb components of all pixels in the stencil
  	double r[blocksize],g[blocksize],b[blocksize];
	...
	for (i=0;i<rows;i++)
	{
		for(j=0;j<columns;j++)
		{
			...
			for(ri = i-hw; ri <= i+hw; ri++) {
				for(rj = j-hw; rj<=j+hw; rj++) {
...
//	FILE* inputFile;
	...
	//loop through whole image
	#pragma omp parallel for default(shared) private(i, j, idx2, temp) reduction(+:r, b, g) 
	for (i=0;i<rows;i++)
	{
		for(j=0;j<columns;j++)
		{
	...
	//Mean values.
	r =(float)r/(float)(rows*columns);
	b =(float)b/(float)(rows*columns);
	g =(float)g/(float)(rows*columns);
	
	//Finding the standard deviation---------------------------
	#pragma omp parallel for default(shared) private(i, j, idx2, temp,r,g,b) reduction(+:rsd, bsd, gsd) 
	for (i=0;i<rows;i++)
	{
		for(j=0;j<columns;j++)
		{
			...
			// traversing through the image.
			temp = im->data + idx2;
			rsd+=(float)(temp->red-r)*(float)(temp->red-r);
			bsd+=(float)(temp->blue-b)*(float)(temp->blue-b);
			gsd+=(float)(temp->green-g)*(float)(temp->green-g);
		}
	}
	...
	//UPDATING THE IMAGE---------------%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	#pragma omp parallel for default(shared) private(i, j, idx2, temp,r,g,b,rsd,bsd,gsd)
	for (i=0;i<rows;i++)
	{
		for(j=0;j<columns;j++)
		{
			...
			// traversing through the image.
			temp = im->data + idx2;
			(out_image->data + idx2)->red = (char)(128*((float)(abs(temp->red-r))/(float)rsd));
			(out_image->data + idx2)->blue = (char)(128*((float)(abs(temp->blue-b))/(float)bsd));
			(out_image->data + idx2)->green = (char)(128*((float)(abs(temp->green-g))/(float)gsd));
		}
	}
